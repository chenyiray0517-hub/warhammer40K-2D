...
> ⚠️ **部署慣例**：每次改 `index.html` 要 deploy 時，**手動把 `<meta name="build">` 的值改新**（如日期+流水號），否則開著遊戲的玩家不會收到更新提示。`sw.js` 若有改也順手 bump `const C='wh40k-vN'`。目前 build = **2026.06.05.2**。
